...
{
    public static class FaceBoxer
    {
        private static readonly Scalar green = new Scalar(0, 255, 0);

        public static void Draw(Mat frame, Point p1, Point p2, float confidence)
        {
            var thickness = (int)(5 * confidence);
            if (thickness <= 0)
                thickness = 1;

            Cv2.Rectangle(frame, p1, p2, green, thickness, LineTypes.Link4);
            ...
            Cv2.PutText(frame, text, pText, HersheyFonts.HersheySimplex, 0.5, green, 2, LineTypes.AntiAlias, false);
